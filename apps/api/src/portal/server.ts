...
export async function createPortal(options:PortalOptions){
 ...
 get('/session',async(req,res)=>{const state=await db.tx(async r=>({membership:await r.get('memberships',req.actor)||{premium:false},deletion_request:req.account?await r.get('deletion_requests',req.actor)||null:null}));res.json({csrf:req.csrf,account:req.account||null,demo,auth_configured:!!(env.SUPABASE_URL&&env.SUPABASE_ANON_KEY),payments_configured:false,commerce:COMMERCE_MODEL,ai_configured:coach.configured,...state});});

...
(async()=>{
 ...
 try{
  ...
  assert.deepEqual({...after.data.recent[0],reconciled_at:undefined},{id:reservation,budget_key:before.data.pending.find(row=>row.id===reservation).budget_key,reserved_cents:5,actual_cents:2,operator_id:'operator',provider_reference:'billing-check-001',reconciled_at:undefined});
  await db.tx(async r=>{const account=await r.get('accounts','operator');await r.put('accounts','operator',{...account,roles:['compliance']});});

...
(async()=>{const db=new LocalStore(':memory:');let paymentCalls=0;const stripe=new Proxy({}, {get(){paymentCalls++;throw Error('Payment SDK must not be used in referral mode');}});
 ...
 const routing=await a('/admin/ai-routing');assert.equal(routing.status,200);assert.equal(routing.data.requests,2);assert(routing.data.routes.some(x=>x.provider==='ollama'));
